<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold font-display text-brand-navy-900">Cuentas por Pagar</h1>
            <p class="text-sm text-brand-soft-textSec">Gestiona tus CxP.</p>
        </div>
        <div>
    ...
    @if($payables->isEmpty())
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-brand-soft-border p-12 flex flex-col items-center justify-center text-center">
            ...
            <p class="text-sm text-brand-soft-textSec mt-2 mb-6 max-w-sm">No se encontraron Cuentas por Pagar en la base de datos.</p>
        </div>
    @else
        <!-- Desktop Table -->
                    ...
                    <tbody class="bg-white divide-y divide-brand-soft-border">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-bold text-brand-navy-900">{{ $payable->id ?? 'N/A' }}</div>
                                <div class="text-[11px] text-brand-soft-textSec">{{ $payable->due_date ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <div class="text-sm text-brand-navy-900">{{ $payable->balance ?? '-' }}</div>
                                <div class="text-[11px] text-brand-soft-textSec">{{ $payable->status ?? '-' }}</div>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="md:hidden space-y-4">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-brand-soft-border p-4 flex flex-col gap-3">
                <div class="flex justify-between items-start">
                    <div>
                        <h4 class="text-sm font-bold text-brand-navy-900">{{ $payable->id ?? 'N/A' }}</h4>
                        <p class="text-xs text-brand-soft-textSec">{{ $payable->due_date ?? '-' }}</p>
                    </div>
                </div>
            </div>
            @endforeach
